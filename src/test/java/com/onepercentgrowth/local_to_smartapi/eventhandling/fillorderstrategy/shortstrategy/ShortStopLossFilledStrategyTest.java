...
class ShortStopLossFilledStrategyTest {
    ...
    @InjectMocks
    private ShortStopLossFilledStrategy strategy;

    // -------------------------------------------------------
    // Scenario 1: SHORT SL fills → cancel target buy, mark trade complete, remove from registry
    // -------------------------------------------------------

    @Test
    void slFills_cancelsTargetBuy_marksTradeComplete() {
        OrderContext ctx = OrderContextFactory.shortWithBuyAndSlOpen(5);
        // BUY type, orderId matches ctx.getStopLossOrderId()
        OrderStatusResponse response = OrderStatusResponseFactory.buyComplete("SL-001", 5, "101.00");
        ...
        verify(executionService, times(1))
                .placeCancelOrder(eq("BUY-001"), anyString(), anyString(), anyString());
    ...
    // -------------------------------------------------------
    // E4: BUY target cancel fails after 3 retries → markInconsistent, tradeCompleted stays false, no registry.remove
    // -------------------------------------------------------

    @Test
    void cancelTargetBuyFails_marksInconsistent_noTradeCompletion() {
        OrderContext ctx = OrderContextFactory.shortWithBuyAndSlOpen(5);
        OrderStatusResponse response = OrderStatusResponseFactory.buyComplete("SL-001", 5, "101.00");
