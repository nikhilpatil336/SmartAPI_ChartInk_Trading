...
class SellFilledOrderStrategyTest {
    ...
    @InjectMocks
    private SellFilledOrderStrategy strategy;

    // -------------------------------------------------------
    // Scenario 1: LONG target fills → cancel SL, mark trade complete, remove from registry
    // -------------------------------------------------------

    @Test
    void targetFills_cancelsSL_marksTradeComplete() {
        OrderContext ctx = OrderContextFactory.longWithSellAndSlOpen(5);
        // SELL type, orderId matches ctx.getSellOrderId()
        OrderStatusResponse response = OrderStatusResponseFactory.sellComplete("SELL-001", 5, "101.00");
    ...
    // -------------------------------------------------------
    // E1: SL cancel fails after 3 retries → markInconsistent, tradeCompleted stays false, no registry.remove
    // -------------------------------------------------------
