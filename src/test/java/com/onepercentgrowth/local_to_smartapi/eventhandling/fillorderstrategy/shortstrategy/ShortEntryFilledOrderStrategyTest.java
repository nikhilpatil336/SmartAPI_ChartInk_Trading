...
class ShortEntryFilledOrderStrategyTest {
    ...
    @InjectMocks
    private ShortEntryFilledOrderStrategy strategy;

    // -------------------------------------------------------
    // Scenario 1: COMPLETE fill — BUY target + SL must be placed
    // (short entry fills → place cover buy + stop-loss buy)
    // -------------------------------------------------------

    @Test
    void completedFill_placesBuyTargetAndSL() {
        OrderContext ctx = OrderContextFactory.freshShort("SELL-001", 5);
        OrderStatusResponse response = OrderStatusResponseFactory.shortEntryComplete("SELL-001", 5, "100.00");
    ...
    void staleWsEvent_deltaZero_doesNotClaimCasOrPlaceOrders() {
        OrderContext ctx = OrderContextFactory.freshShort("SELL-001", 5);
        ctx.getLastSellFilledQty().set(5); // already processed qty=5
        OrderStatusResponse response = OrderStatusResponseFactory.shortEntryComplete("SELL-001", 5, "100.00");
    ...
    // -------------------------------------------------------
    // Scenario 5: sell is open on exchange but no partial fill was processed
    //   Unique guard in ShortEntryFilledOrderStrategy:
    //   !isSellPartiallyFilled() && isSellOpen() → return empty before CAS is touched
    // -------------------------------------------------------

    @Test
    void sellOpenButNotPartialFilled_returnsEmpty() {
        OrderContext ctx = OrderContextFactory.shortSellOpenNotPartialFilled("SELL-001", 5);
        OrderStatusResponse response = OrderStatusResponseFactory.shortEntryComplete("SELL-001", 5, "100.00");
