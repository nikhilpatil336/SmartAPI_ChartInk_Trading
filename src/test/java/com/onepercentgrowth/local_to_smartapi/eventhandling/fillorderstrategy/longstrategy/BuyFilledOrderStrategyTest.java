...
class BuyFilledOrderStrategyTest {
    ...
    @InjectMocks
    private BuyFilledOrderStrategy strategy;

    // -------------------------------------------------------
    // Scenario 1: COMPLETE fill — SL + target must be placed
    // -------------------------------------------------------

    @Test
    void completedFill_placesSellAndSL() {
        ...
        when(applicationProperties.getLeverageMultiplierToUseForLong()).thenReturn(5);
    ...
    // -------------------------------------------------------
    // Scenario 3: CAS already claimed (race — open partial arrived first)
    //   complete event arrives, delta > 0, but CAS taken → no duplicate orders
    // -------------------------------------------------------
    ...
    void staleWsEvent_deltaZero_doesNotClaimCasOrPlaceOrders() {
        OrderContext ctx = OrderContextFactory.freshLong("BUY-001", 5);
        ctx.getLastBuyFilledQty().set(5); // already processed qty=5
        OrderStatusResponse response = OrderStatusResponseFactory.buyComplete("BUY-001", 5, "100.00");
