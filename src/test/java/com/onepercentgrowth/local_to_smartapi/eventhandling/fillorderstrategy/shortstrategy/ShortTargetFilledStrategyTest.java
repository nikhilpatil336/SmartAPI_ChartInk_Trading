...
class ShortTargetFilledStrategyTest {
    ...
    @InjectMocks
    private ShortTargetFilledStrategy strategy;

    // -------------------------------------------------------
    // Scenario 1: SHORT target (cover buy) fills → cancel SL, mark trade complete, remove from registry
    // -------------------------------------------------------

    @Test
    void targetFills_cancelsSL_marksTradeComplete() {
        OrderContext ctx = OrderContextFactory.shortWithBuyAndSlOpen(5);
        // BUY type, orderId matches ctx.getBuyOrderId()
        OrderStatusResponse response = OrderStatusResponseFactory.buyComplete("BUY-001", 5, "99.00");
        ...
        verify(executionService, times(1))
                .placeCancelOrder(eq("SL-001"), anyString(), anyString(), anyString());
    ...
    // -------------------------------------------------------
    // E3: SL cancel fails after 3 retries → markInconsistent, tradeCompleted stays false, no registry.remove
    // -------------------------------------------------------

    @Test
    void cancelSlFails_marksInconsistent_noTradeCompletion() {
        OrderContext ctx = OrderContextFactory.shortWithBuyAndSlOpen(5);
        OrderStatusResponse response = OrderStatusResponseFactory.buyComplete("BUY-001", 5, "99.00");
